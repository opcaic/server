...
namespace OPCAIC.ApiService.Security
{
	public static class AuthorizationConfiguration
	{
		public static void Setup(AuthorizationOptions options)
		{
			...
			AddPolicy(options, RolePolicy.Organizer, nameof(UserRole.Admin), nameof(UserRole.Organizer));
			AddPolicy(options, RolePolicy.User, nameof(UserRole.Admin), nameof(UserRole.Organizer), nameof(UserRole.User));
		...
		{
			options.AddPolicy(policyName, policy => policy.RequireClaim(RolePolicy.PolicyName, requiredValues));
		}
	}
}
