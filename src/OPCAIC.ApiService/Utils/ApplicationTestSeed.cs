...
namespace OPCAIC.ApiService.Utils
{
	public class ApplicationTestSeed
	{
		...
		/// <inheritdoc />
		public ApplicationTestSeed(IServiceProvider services)
		{
		...
		private void DoSeed()
		{
			...
			{
				foreach (var directory in Directory.GetDirectories(conf.Directory))
				{
			...
			userManager.CreateAsync(userOrganizer, "Password").Wait();
			userOrganizer = context.Users.Find(userOrganizer.Id);
			userManager.CreateAsync(user, "Password").Wait();
			user = context.Users.Find(user.Id);
			userManager.CreateAsync(userB, "Password").Wait();
			...
			var tournamentSokoban = new Tournament
			{
				...
				MaxSubmissionSize = 100 * 1024,
				Description =
					"Sokoban is a type of puzzle video game, in which the player pushes crates or boxes around in a warehouse, trying to get them to storage locations. Sokoban was created in 1981 by Hiroyuki Imabayashi, and published in December 1982 by Thinking Rabbit, a software house based in Takarazuka, Japan.",
				MenuItems =
					new List<MenuItem>
					{
			...
			CreateSubmissions(users, tournamentChessTable);
			...
			var submissionChessOrganizer = CreateSubmission(userOrganizer, tournamentChessElo,
				Randomizer.Seed.NextDouble() * 1500);
			context.SaveChanges();
			CreateSubmissions(users.OrderBy(u => u.Id).Skip(3).ToList(), tournamentChessElo);
			CreateSubmissions(users, tournamentChessTable);
			CreateSubmissions(users, tournamentChessDe);
			...
			AddExecution(matchAdminOrganizer, EntryPointResult.Success, DateTime.Now.AddDays(-1));
			var matchUsers = CreateMatch(tournamentDotaSe, 1, submissionDotaUser,
				submissionDotaUserB);
			AddExecution(matchUsers, EntryPointResult.Success, DateTime.Now.AddDays(-2));
			var final = CreateMatch(tournamentDotaSe, 2, submissionDotaOrganizer,
				submissionDotaUserB);
			AddExecution(final, EntryPointResult.Success, DateTime.Now);
			#endregion
		...
		private void CreateSubmissions(IEnumerable<User> users, Tournament tournament)
		{
		...
		private Match CreateMatch(Tournament tournament, int index,
		...
		private MatchExecution AddExecution(Match match,
			...
			DateTime? executed = null)
		{
			...
			var matchExecution = new MatchExecution
			{
				...
				Executed = executed ?? DateTime.Now
			};
		...
		private Submission CreateSubmission(User author,
			Tournament tournament, double score)
		{
			...
			if (participation == null)
			{
				...
				{
					Tournament = tournament, User = author, Submissions = new List<Submission>()
				};
