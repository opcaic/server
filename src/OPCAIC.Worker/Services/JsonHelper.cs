...
namespace OPCAIC.Worker.Services
{
	public static class JsonHelper
	{
		...
		private static readonly JsonSerializerSettings withExtraSettings =
			new JsonSerializerSettings
			{
				ContractResolver = new StrictContractResolver(), CheckAdditionalContent = true
			};
		...
		/// <param name="json">The JSON to deserialize.</param>
		/// <returns></returns>
		public static T DeserializeWithExtra<T>(string json)
		{
			return JsonConvert.DeserializeObject<T>(json, withExtraSettings);
		}
